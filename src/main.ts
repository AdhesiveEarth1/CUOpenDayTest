    ...
    <div class="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  px-2">
      ${topics.map((topic: any) => `
        <div class="bg-cardiff-red rounded-lg shadow p-6 flex flex-col">
          ...
          <button id="subjectButton" onclick= "sessionStorage.setItem('Subject', JSON.stringify([${topic.id}]))"> Show Event Info </button>
        </div>
      `).join('')}
    </div>
  `;
...
function renderIndividualTopicInfo(topic: any){
  topic = localStorage.getItem('Subject')
